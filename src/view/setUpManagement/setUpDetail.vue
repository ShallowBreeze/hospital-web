  ...
  <div class="out-box">
    ...
    <div class="top-row-box">
      <div class="info-in-box margin-right">
        ...
        <el-select v-model="selectHospitalID" placeholder="请选择" style="width: 70%" @change="getHospitalDepartmentList">
      ...
      <div class="info-in-box margin-right">
        ...
        <el-select v-model="selectDepartmentID" placeholder="请选择" style="width: 70%" @change="getOutpatientByHospital">
          <el-option v-for="item in departmentSelectData" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </div>
      ...
      <div class="info-in-box margin-right">
        ...
        <el-select v-model="selectOutpatientID" placeholder="请选择" style="width: 70%">
          <el-option v-for="item in outpatientSelectData" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </div>

      <div class="info-in-box margin-right">
        <span class="title">日期：</span>
        <el-date-picker v-model="dateOutCall" type="date" placeholder="选择日期"></el-date-picker>
      </div>
    ...
    <!--     添加出诊计划弹出框-->
    <el-dialog title="添加" :visible.sync="dialogFormVisible" width="35%" @close="cancelModal" v-loading="isLoading">
      <el-form>
        ...
        <el-form-item label="医生姓名：" label-width="120px">
          <el-select v-model="selectDoctorID" placeholder="请选择" style="width: 70%">
            <el-option v-for="item in doctorSelectData" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="诊室：" label-width="120px">
          <el-select v-model="selectOutCallID" placeholder="请选择" style="width: 70%">
            <el-option v-for="item in sentTableData.address" :key="item.id" :label="item.address" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="时间段：" label-width="120px">
          ...
          <!-- <el-select v-model="selectTimeID" placeholder="请选择" style="width: 70%">
            <el-option v-for="item in sentTableData.time" :key="item.ID" :label="item.name" :value="item.ID"></el-option>
          </el-select> -->
        </el-form-item>
      </el-form>
    ...
    <!--     修改出诊计划弹出框-->
    <el-dialog title="修改" :visible.sync="updateDialog" width="35%" @close="cancelModal" v-loading="isLoading">
      <el-form>
        <el-form-item label="医生ID：" label-width="120px">
...
export default {
  ...
  data() {
    return {
      ...
      interval: '00:30', // 排班间隔时间
      startTime: '', // 添加表单排班开始时间
      endTime: '', // 添加表单排班结束时间
  ...
  methods: {
    ...
    getOutpatientByHospital: function () {
      ...
      getOutpatientByHospital(this.selectHospitalID, this.selectDepartmentID, 1, 50)
        .then((res) => {
          if (res.code === 200) {
    ...
    getAllOutCallList: function () {
      ...
      getAllOutCallList(dateFormYMD(this.dateOutCall), this.pageList.pageNum, this.pageList.pageSize, this.selectHospitalID, this.selectDepartmentID, this.selectOutpatientID)
        .then((res) => {
          if (res.code === 200) {
            const data = res.data.list;
            let that = this;
            if (data.length > 0) {
              data.forEach(function (item, index) {
                that.tableAllData.tableData.push({
                  ...
                  time: outCallTime(item.startTime, item.endTime),
                  timeId: item.time
                });
              });
              // this.selectDoctorID = '';
    ...
    getDoctorList: function () {
      ...
      getDoctorList(this.selectDepartmentID, this.selectOutpatientID, 1, 100, '')
        .then((res) => {
          if (res.code === 200) {
            if (res.data.list.length > 0) {
